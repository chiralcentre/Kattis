...
public class integerlists {
    public static void main(String[] args) throws IOException{
        ...
        for (int i = 0; i < T; i++){
            ...
            ArrayList<String> newlst = new ArrayList<>(lst);
            if (error){pw.println("error");}
            else{
                pw.print("[");
                if (reverse){
                    for (int k = n-1; k >= 0; k--){
                        if (k > 0) pw.print(newlst.get(k)+","); //comma separator
                        else pw.print(newlst.get(k));
                    }
                }
                else{
                    for (int k = 0; k < n; k++){
                        if (k < n - 1) pw.print(newlst.get(k)+","); //comma separator
                        else pw.print(newlst.get(k));
                    }
                }
                pw.print("]\n");
