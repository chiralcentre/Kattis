...
public class continuousmedian {
    public static void main(String[] args) throws IOException{
        ...
        for (int i = 0; i < T; i++){ // O(N log N) per test case
            int N = Integer.parseInt(br.readLine()); String[] A = br.readLine().split(" ");
            IntegerAVL tree = new IntegerAVL(); long medianSum = 0;
            for (int j = 0; j < N; j++){
                tree.insert(Integer.parseInt(A[j]));
                if (j%2 == 0) medianSum += tree.select((j+2)/2);
                else {
                    int p1 = (j+2)/2; int p2 = p1 + 1;
                    medianSum += ((tree.select(p1) + tree.select(p2))/2);
                }
            }
            pw.println(medianSum);
        }
        br.close();
        pw.close();
    }
