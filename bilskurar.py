...
arr1 = list(map(lambda x: int(x) - 1,stdin.readline().split()))
arr2 = list(map(lambda x: int(x) - 1,stdin.readline().split()))
c = [0 for _ in range(n)]
# c[x] = y means that arr1[y] = x, reverse mapping for arr1
for i in range(n):
    c[arr1[i]] = i
stdout.write(f"{sortAndCountInv(0,n - 1,[c[arr2[i]] for i in range(n)])}\n")
