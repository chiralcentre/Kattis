...
class stringAVL {
    ...
    // Return v if it is found in the BST otherwise return null
    ...
    public String predecessor(AVLVertex T) {
    ...
    // public method to find rank of a nickname containing key with value v from BST
    ...
    // helper method 
    private int rank(AVLVertex T, String v) {
        if (T == null) return 0;
        int cmp = T.key.compareTo(v);
        if (cmp > 0) return rank(T.left, v);
        else if (cmp < 0) return 1 + size(T.left) + rank(T.right,v);
...
public class nicknames {
    public static void main(String[] args) throws IOException {
        ...
        stringAVL names = new stringAVL();
        for (int i = 0; i < A; i++){names.insert(br.readLine().strip());} 
        int B = Integer.parseInt(br.readLine());
        for (int j = 0; j < B; j++){
            String nickname = br.readLine().strip();
            //nickname with last character amended to character with ascii value + 1; this operation can be taken to be carried out in O(1) time since nicknames do not get longer than 10 characters
            String higherNickname = nickname.substring(0,nickname.length()-1) + Character.toString(((int) nickname.charAt(nickname.length()-1) + 1)); 
            int a = names.rank(nickname), b = names.rank(higherNickname);
            pw.println(b-a);
        }
        br.close();
        pw.close();
