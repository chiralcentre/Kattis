...
EPSILON = pow(10,-7)
def inRange(a,b,c):
    return min(b,c) - EPSILON <= a <= max(b,c) + EPSILON
...
def segments_intersect(x1,y1,x2,y2,x3,y3,x4,y4):
    ...
    if firstLineIsPoint and not secondLineIsPoint:
        return inRange(x1,x3,x4) and inRange(y1,y3,y4) and (x4 - x3) * y2 + (y4 - y3) * x3 == (y4 - y3) * x2 + y3 * (x4 - x3)
    elif not firstLineIsPoint and secondLineIsPoint:
        return inRange(x3,x1,x2) and inRange(y3,y1,y2) and (x2 - x1) * y4 + (y2 - y1) * x1 == (y2 - y1) * x4 + y1 * (x2 - x1)
        ...
        if dx2 * dy1 == dy2 * dx1:
            ...
            if dx1 == 0 or dx2 == 0:
                ...
                xa = (c2 - c1) / (g1 - g2)
                ya = g1 * xa + c1
            return inRange(xa,x1,x2) and inRange(xa,x3,x4) and inRange(ya,y1,y2) and inRange(ya,y3,y4)

segments = []
# note that each coordinate is in the range (0,10000)
# to simulate an infinite line, add 10000 times the change in x and y to both endpoints
...
for i in range(int(stdin.readline())):
    ...
    for x1,y1,x2,y2 in segments:
        if segments_intersect(x1,y1,x2,y2,x3,y3,x4,y4):
            crossed += 1
    stdout.write("different\n") if crossed % 2 else stdout.write("same\n")
